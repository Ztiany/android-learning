...
public final class LiveEventBusCore {
    ...
    private class LiveEvent<T> implements Observable<T> {
        ...
        /**
         * 以广播的形式发送一个消息，需要跨进程、跨 APP 发送消息的时候调用该方法
         *
         * @param value      发送的消息
        ...
        private void broadcastInternal(T value, boolean foreground, boolean onlyInApp) {
            logger.log(Level.INFO, "broadcast: " + value + " foreground: " + foreground + " with key: " + key);
            Application application = AppUtils.getApp();
            if (application == null) {
            ...
            if (foreground && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                ...
                During normal broadcasts the receivers are not automatically hoisted out of the background priority class.
                 */
        ...
        private void observeInternal(@NonNull LifecycleOwner owner, @NonNull Observer<T> observer) {
            ...
            // 防止在注册时收到消息，这是因为 LiveData 的特性，它会在注册时收到最新的消息。
            ...
            logger.log(Level.INFO, "observe observer: " + observerWrapper + "(" + observer + ")" + " on owner: " + owner + " with key: " + key);
        }
        ...
        private void observeStickyInternal(@NonNull LifecycleOwner owner, @NonNull Observer<T> observer) {
            ObserverWrapper<T> observerWrapper = new ObserverWrapper<>(observer);
            liveData.observe(owner, observerWrapper);
            logger.log(Level.INFO, "observe sticky observer: " + observerWrapper + "(" + observer + ")" + " on owner: " + owner + " with key: " + key);
        }
        ...
        private void observeForeverInternal(@NonNull Observer<T> observer) {
            ...
            logger.log(Level.INFO, "observe forever observer: " + observerWrapper + "(" + observer + ")" + " with key: " + key);
        }
        ...
        private void observeStickyForeverInternal(@NonNull Observer<T> observer) {
            ...
            observerMap.put(observer, observerWrapper);
            liveData.observeForever(observerWrapper);
            logger.log(Level.INFO, "observe sticky forever observer: " + observerWrapper + "(" + observer + ")" + " with key: " + key);
        }
