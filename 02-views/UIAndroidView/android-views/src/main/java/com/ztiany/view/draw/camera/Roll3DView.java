...
public class Roll3DView extends View {
    ...
    private void initIndex() {
        ...
        //因为旋转可以往前也可以往后
        if (nextIndex > listSize - 1)//如果到了最后一张，下一张就显示第一张
            nextIndex = 0;
        if (preIndex < 0)//如果到了第一张，上一张就显示最后一张
            preIndex = listSize - 1;
    }
    ...
    private void drawRollWhole3D(Canvas canvas, boolean draw2D) {
        ...
        Log.d("Roll3DView", "getMeasuredWidth():" + getMeasuredWidth());
        Log.d("Roll3DView", "getMeasuredHeight():" + getMeasuredHeight());
        Log.d("Roll3DView", "rotateDegree:" + rotateDegree);
        Log.d("Roll3DView", "axisY:" + axisY);

        if (direction == 1) { //1竖直方向
            ...
            camera.restore();
            //水平方向在中心点，而垂直方向当中心点始终在改变 axisY在此处的变化范围是：0 -> viewHeight
            //变换轴心：上顶点到下顶点
            matrix.preTranslate(-viewWidth / 2, 0);
            matrix.postTranslate(viewWidth / 2, axisY);//旋转的中心axisY
