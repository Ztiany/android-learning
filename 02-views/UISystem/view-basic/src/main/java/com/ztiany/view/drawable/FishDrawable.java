...
public class FishDrawable extends Drawable {
    ...
    private void init() {
        ...
        //为了保证 -1 到 - 的完整周期，ValueAnimator.ofFloat 的参数必须是公倍数
        // 1.2  1.5 ==》 10
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        float fishAngle = (float) (fishMainAngle + Math.sin(Math.toRadians(currentValue * 1.2 * frequency)) * 10/*鱼头摆动幅度*/);
        ...
        float findEdgeLength = (float) Math.abs(Math.sin(Math.toRadians(currentValue * 1.5 * frequency)) * BIG_CIRCLE_RADIUS);
    ...
    private void makeTriangle(Canvas canvas, PointF startPoint, float findCenterLength, float findEdgeLength, float fishAngle) {
        ...
        float triangleAngle = (float) (fishAngle + Math.sin(Math.toRadians(currentValue * 1.5 * frequency)) * 35);
