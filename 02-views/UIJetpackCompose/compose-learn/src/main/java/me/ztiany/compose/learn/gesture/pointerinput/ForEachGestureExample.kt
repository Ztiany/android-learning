...
当组件离开视图树时，手势监听也会随之结束。
    ...
    Column(
        ...
        Text(
            ...
            通过 forEachGesture 的源码可知，每一轮手势处理结束后，或本次手势处理被取消时，都会使用 awaitAllPointersUp() 保证所有手指均已抬起（一轮的手势已经结束，从 Down 到 Move 再到 Up），并且同时也会与当前组件的生命周期对齐，当组件离开视图树时，手势监听也会随之结束。
