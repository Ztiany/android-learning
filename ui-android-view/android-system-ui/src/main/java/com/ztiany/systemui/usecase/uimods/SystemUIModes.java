...
package com.ztiany.systemui.usecase.uimods;
...
public class SystemUIModes extends Activity implements OnQueryTextListener, ActionBar.TabListener {
    ...
    private void setFullscreen(boolean on) {
        ...
        //设置全屏
        final int bits = WindowManager.LayoutParams.FLAG_FULLSCREEN;// api 1
        if (on) {
    ...
    private void setOverscan(boolean on) {
        ...
        WindowManager.LayoutParams winParams = win.getAttributes();
        //允许窗口内容扩展到屏幕上的扫描区域
        final int bits = WindowManager.LayoutParams.FLAG_LAYOUT_IN_OVERSCAN;// api 18
        if (on) {
            winParams.flags |= bits;
        } else {
            winParams.flags &= ~bits;
    ...
    private void setTranslucentStatus(boolean on) {
        ...
        WindowManager.LayoutParams winParams = win.getAttributes();
        //半透明状态栏
        final int bits = WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS;// api 19
        if (on) {
            winParams.flags |= bits;
        } else {
            winParams.flags &= ~bits;
    ...
    private void setTranslucentNavigation(boolean on) {
        Window win = getWindow();
        WindowManager.LayoutParams winParams = win.getAttributes();
        final int bits = WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION;  // api 19
        if (on) {
            winParams.flags |= bits;
        } else {
            winParams.flags &= ~bits;
    ...
    int[] mCheckFlags = new int[]{
            ...
            View.SYSTEM_UI_FLAG_FULLSCREEN,//控制StatusBar的显示，同时还会隐藏ActionBar
            View.SYSTEM_UI_FLAG_HIDE_NAVIGATION,//控制NavigationBar的显示

            View.SYSTEM_UI_FLAG_IMMERSIVE,              // API 19 沉浸式
            View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY,// API 19 粘性沉浸式

            View.SYSTEM_UI_FLAG_LAYOUT_STABLE,//固定布局的大小，不因SystemBar的显示与隐藏而发生变化
            View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN,//让布局UI延伸到状态栏下方，状态栏显示在布局的上方，并挡住了部分布局
            View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION//让布局UI延伸到导航栏下方，导航栏显示在布局的上方，并挡住了部分布局
    };
