...
public class RulerView extends View {
    ...
    //手势相关参数
    private int mBeginRange;//开始
    private int mEndRange;//结束
    private int mLastMotionX;//位置
    private boolean mIsDragged;

    private int mGravity;
    private int mInnerWidth;//宽度
    private int mIndicatePadding;//刻度padding
    private Paint mIndicatePaint;//画刻度
    private Paint mTextPaint;//画文本
    private int mIndicateWidth;//标度宽度
    private float mIndicateScale;
    private boolean mIsAutoAlign = true;//自动对齐
    ...
    public RulerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mIndicateColor = ta.getColor(R.styleable.RulerView_rv_indicateColor, Color.BLACK);
        mTextColor = ta.getColor(R.styleable.RulerView_rv_textColor, Color.GRAY);
        mTextSize = ta.getDimension(R.styleable.RulerView_rv_textSize, 18);
        mBeginRange = ta.getInt(R.styleable.RulerView_rv_begin, 0);
        mEndRange = ta.getInt(R.styleable.RulerView_rv_end, 100);
        mIndicateWidth = (int) ta.getDimension(R.styleable.RulerView_rv_indicateWidth, 5);
        mIndicatePadding = (int) ta.getDimension(R.styleable.RulerView_rv_indicatePadding, 15);
        ta.recycle();
