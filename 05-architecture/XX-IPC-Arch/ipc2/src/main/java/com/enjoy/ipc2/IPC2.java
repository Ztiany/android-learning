...
public class IPC2 {
    ...
    public static <T> T getInstance(Class<? extends IPCService> service, Class<T> classType, Object... parameters) {
        return getInstanceWithName(service, classType, "getInstance", parameters);
    }

    public static <T> T getInstanceWithName(Class<? extends IPCService> service, Class<T> classType, String methodName, Object... parameters) {
        if (!classType.isInterface()) {
            //抛异常！
        }
        ServiceId serviceId = classType.getAnnotation(ServiceId.class);
        Response response = Channel.getInstance().send(Request.GET_INSTANCE, service, serviceId.value(), methodName, parameters);
        if (response.isSuccess()) {
            return (T) Proxy.newProxyInstance(classType.getClassLoader(), new Class[]{classType}, new IPCInvocationHandler(service, serviceId.value()));
        }
        return null;
    }
