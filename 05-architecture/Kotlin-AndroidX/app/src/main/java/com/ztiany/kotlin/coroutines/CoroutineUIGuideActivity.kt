...
import com.ztiany.kotlin.databinding.ActivityCoroutineGuideBinding
...
class CoroutineUIGuideActivity : AppCompatActivity() {
    ...
    private fun bindLifecycle(): Job {
        if (completableJob.isCancelled || completableJob.isCompleted) {
            completableJob = Job()
        }
    ...
    fun startCounter(view: View) {
        GlobalScope.launch(Dispatchers.Main + bindLifecycle()) {
            ...
            for (i in 10 downTo 1) { // 从 10 到 1 的倒计时
                binding.tvCoroutineGuideValue.text = "Countdown $i ..." // 更新文本
                delay(500) // 等待半秒钟
            }
            binding.tvCoroutineGuideValue.text = "Done!"
        }
    }
    ...
    private fun setupActorViews() {
        binding.btnCoroutineGuideActor1.onClick {
            for (i in 10 downTo 1) { // 从 10 到 1 的倒计时
                binding.tvCoroutineGuideValue.text = "Countdown $i ..." // 更新文本
                delay(500) // 等待半秒钟
            }
        ...
        binding.btnCoroutineGuideActor2.onClickConflated {
            for (i in 10 downTo 1) { // 从 10 到 1 的倒计时
                binding.tvCoroutineGuideValue.text = "Countdown $i ..." // 更新文本
                delay(500) // 等待半秒钟
            }
    ...
    fun startUiBlockProblem(view: View) {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            var counter = 0
            while (true) {
                binding.tvCoroutineGuideValue.text = "${++counter}: $result"
        ...
        binding.btnCoroutineGuideActor4.onClickConflated {
        ...
        // 它会饱和占用最多一个 CPU 核心。
        binding.btnCoroutineGuideActor5.onClickConflated {
            result = "fib($x) = ${fib(x)}"
            x++
        }
    }
    ...
    private fun setupAdvanceViews() {
        ...
        After delay
         */
        binding.btnCoroutineGuideActor6.setOnClickListener {
        ...
        After delay
         */
        binding.btnCoroutineGuideActor7.setOnClickListener {
            println("Before launch")
