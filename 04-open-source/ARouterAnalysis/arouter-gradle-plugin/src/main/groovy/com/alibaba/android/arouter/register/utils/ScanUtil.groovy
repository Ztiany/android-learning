...
class ScanUtil {
     ...
     * scan jar file
     *
     * RegisterTransform.registerList  之类的接口的实现类，都收集起来。
     *
    ...
    static void scanJar(File jarFile, File destFile) {
        if (jarFile) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                /* 只处理 com/alibaba/android/arouter/routes/ 类路劲下的类*/
                if (entryName.startsWith(ScanSetting.ROUTER_CLASS_PACKAGE_NAME)) {
                    InputStream inputStream = file.getInputStream(jarEntry)
                    scanClass(inputStream)
                    inputStream.close()
                } else if (ScanSetting.GENERATE_TO_CLASS_FILE_NAME == entryName) {
                    /*找到要改写方法的哪个类（即 com/alibaba/android/arouter/core/LogisticsCenter）所在的 file*/
